...
subroutine el_Cahn_Hilliard_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real (prec)  ::  dd, kk,theta, D44, D11, D12              ! Material properties
    real (prec)  ::  q(6),fc,dfdc
    real (prec)  ::  uu(6),duu(6)
    !
    ...
    do kint = 1, n_points
        ...
        !write (*,*) 'uu=',uu(1)
        fc = (uu(2)+duu(2))*((uu(2)+duu(2))**(2.d0)-1.d0)
        !fdc = duu(2)*(duu(2)**(2.d0)-1)
        !write (*,*)'fc=',fc
        q(1) = uu(1)+duu(1)-fc
        q(2) = duu(2)/DTIME
        q(3) = -kk*(uu(5)+duu(5))
        q(4) = -kk*(uu(6)+duu(6))
        q(5) = dd*(uu(3)+theta*duu(3))
        q(6) = dd*(uu(4)+theta*duu(4))

        dfdc = 3*(uu(2)+duu(2))**(2.d0)-1.d0
        !D = 0.d0
        D(1,1) = 1.d0
        D(1,2) = -dfdc
        D(2,2) = 1/DTIME
        ...
        !write (*,*)'element_stiffness=',element_stiffness(1,1)
    end do
...
!subroutine el_Cahn_Hilliard_2dbasic_dynamic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
!    n_properties, element_properties,element_coords, length_coord_array, &                               ! Input variables
!    dof_increment, dof_total, length_dof_array,  &                                                       ! Input variables
!    n_state_variables, initial_state_variables, &                                                        ! Input variables
...
!
!    return
!end subroutine el_Cahn_Hilliard_2dbasic_dynamic
!
!
!==========================SUBROUTINE fieldvars_linelast_2dbasic ==============================
!subroutine fieldvars_Cahn_Hilliard_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
!    n_properties, element_properties,element_coords,length_coord_array, &                                ! Input variables
!    dof_increment, dof_total, length_dof_array,  &                                                      ! Input variables
...
!
!    real (prec)  ::  strain(3), dstrain(3)             ! Strain vector contains [e11, e22, e33, 2e12, 2e13, 2e23]
!    real (prec)  ::  stress(3),s3                      ! Stress vector contains [s11, s22, s33, s12, s13, s23]
...
!        B(3,2:3*n_nodes:2) = dNdx(1:n_nodes,1)
!
!        strain = matmul(B,dof_total)
!        dstrain = matmul(B,dof_increment)
!        stress = matmul(D,strain+dstrain)
...
!        smises = dsqrt(((stress(1)-stress(2))**2+(stress(1)-s3)**2+(stress(2)-s3)**2+6.d0*stress(3)**2)/2.d0)
...
!
!    return
!end subroutine fieldvars_Cahn_Hilliard_2dbasic
!
